...
describe('packages/osv-offline-db/src/lib/db.int', () => {
  ...
  const containerVuln: Vulnerability & { _id: string } = {
    ...
    related: ['CGA-2ph7-wp75-g3rf', 'CGA-326j-45xp-qqrg', 'CGA-3727-xg6m-m6g6'],
  ...
  describe('hot reload', () => {
    ...
    it('works for RPMs', async () => {
      using db = await createDbWithContent(
        'rpm.nedb',
        JSON.stringify(sampleRpmVuln)
      );
      const result = await db.query('RPM', 'kernel-headers');
  ...
  describe('query_containers', () => {
    it('works', async () => {
      using db = await createDbWithContent(
        'docker.nedb',
        JSON.stringify(containerVuln)
      );
      const result = await db.query_containers(
    ...
    it('returns empty array for invalid package', async () => {
      ...
      const result = await db.query_containers('this-package-doesnt-exist');
